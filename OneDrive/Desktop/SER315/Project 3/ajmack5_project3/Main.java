...
public class Main {
	...
	public static void main(String[] args) {
		...
		time.set(2020, 9, 5);
		Race r0 = (new Race("Spartan", 8, 3, new Date(time.getTimeInMillis()), Type.OBSTACLE, rom.organizer));
		rom.races.add(r0);

		time.set(2019, 11, 5);
		Race r1 = new Race("ToughestMudder", 20, 3, new Date(time.getTimeInMillis()), Type.OBSTACLE, rom.organizer);
		rom.races.add(r1);

		time.set(2020, 11, 7);
		Race r2 = new Race("ToughMudder", 9, 1, new Date(time.getTimeInMillis()), Type.OBSTACLE, rom.organizer);
		rom.races.add(r2);

		time.set(2020, 1, 1);
		Race r3 = new Race("ElephantMountain", 12, 5, new Date(time.getTimeInMillis()), Type.TRAIL, rom.organizer);
		rom.races.add(r3);

		//		print the race dates and names
		System.out.println("\n======================");
		System.out.println("Print races");
		for (Race race : rom.races) {
		...
		Team t1 = new Team("CodeDevils",rom.races.get(0), rpm.members.get(0));
		r0.teams.add(t1);
		julie.teams.add(t1);
		...
		System.out.println(rpm.register_for_race("david@david.com", r2.date) + " == race_already_full"); // I am reusing the date directly from the race, so this is a bit of a cheat to not have to define it again
		System.out.println(rpm.register_for_race("jacob@jacob.com", r1.date) + " == insufficient_credits"); // I am reusing the date directly from the race, so this is a bit of a cheat to not have to define it again
		System.out.println(rpm.register_for_race("ryan@ryan.com", r1.date) + " == already_registered"); // I am reusing the date directly from the race, so this is a bit of a cheat to not have to define it again
		System.out.println(rpm.register_for_race("Julie@julie.com", r0.date) + " == already_registered"); // I am reusing the date directly from the race, so this is a bit of a cheat to not have to define it again
		System.out.println(rpm.register_for_race("Julie@julie.com", r1.date) + " == registration_successful"); // I am reusing the date directly from the race, so this is a bit of a cheat to not have to define it again
